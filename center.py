...
class Main:
    def __init__(self):
        ...
        self.title = tk.Label(self.root, text='Add Center', font=("Arial", '20', 'bold'))
        ...
        self.lb2 = tk.Label(self.formFrame, text='Enter Email', font=self.formFont)
        self.lb2.grid(row=1, column=0, padx=10, pady=10)
        self.txt2 = tk.Entry(self.formFrame, font=self.formFont)
        self.txt2.grid(row=1, column=1, padx=10, pady=10)

        self.lb3 = tk.Label(self.formFrame, text='Enter Mobile', font=self.formFont)
        ...
        self.lb4 = tk.Label(self.formFrame, text='Enter Password', font=self.formFont)
        self.lb4.grid(row=3, column=0, padx=10, pady=10)
        self.txt4 = tk.Entry(self.formFrame, show='*', font=self.formFont)
        self.txt4.grid(row=3, column=1, padx=10, pady=10)

        self.lb5 = tk.Label(self.formFrame, text='Select State', font=self.formFont)
        ...
        self.lb6 = tk.Label(self.formFrame, text='Select City', font=self.formFont)
    ...
    def getstate(self):
        q = f"select distinct state from city"
        self.cr.execute(q)
    ...
    def update_cities(self, event):
        ...
        q = f"select city from city where state = '{selected_state}'"
        self.cr.execute(q)
        rows = self.cr.fetchall()
    ...
    def getFormValues(self):
        ...
        self.cr.execute(q)
        if self.cr.fetchall():
            msg.showwarning("Warning", "Email already exists.")
            return

        q = f"select * from center where mobile='{self.mobile}'"
        self.cr.execute(q)
        if self.cr.fetchall():
            msg.showwarning("Warning", "Mobile already exists.")
            return
