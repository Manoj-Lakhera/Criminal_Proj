...
class Main:
    def __init__(self):
        ...
        self.title = tk.Label(self.root, text='Add new Location', font=("Arial", '20', 'bold'))
        ...
        self.lb1 = tk.Label(self.formFrame, text='Select State', font=self.formFont)
        ...
        self.lb2 = tk.Label(self.formFrame, text='Select City', font=self.formFont)
    ...
    def getFormValues(self):
        ...
        if self.state == 'Select state' or self.city == 'Select city':
            ...
            q = f"insert into city (state, city) values ('{self.state}', '{self.city}')"
            self.cr.execute(q)
            self.conn.commit()
            self.conn.close()
            msg.showinfo('Success', 'location has been Added')
            self.resetForm()
